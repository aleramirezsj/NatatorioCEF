...
namespace NatatorioCEF.AdminData
{
    public class RepositorySocios : IRepository
    {
        ...
        {
            return db.Socios.Include(l=> l.Localidad).ToList();
        }
        public IEnumerable<object> GetAll(string busqueda)
        {
            return db.Socios.Include(l=> l.Localidad).Where(c => c.Apellido.Contains(busqueda) || c.Nombre.Contains(busqueda) || c.DNI.ToString().Contains(busqueda)).ToList();
        }
        ...
        {
            var socio = db.Socios.Include(l=> l.Localidad).FirstOrDefault(s=> s.Id ==id);
            return socio;
        }
    }
}
